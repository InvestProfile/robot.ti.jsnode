...
<body>
  ...
  <script>
    ...
    async function load() {
      ...
      document.getElementById('socialProfiles').innerHTML = rows(['Status', 'Profile', 'Manual', 'Auto', 'Effective', 'Signals', 'Return', 'Checked', 'Reason'], socialCollector.profiles, p =>
        '<tr><td>' + (p.status === 'ready' ? pill('good', p.status) : pill('warn', p.status)) + '</td><td>' + esc(p.displayName || p.profileKey) + '<div class="small">' + esc(p.profileUid || p.profileUrl) + '</div><div class="small">followers ' + esc(p.followersCount) + ', ops30d ' + esc(p.monthOperationsCount) + '</div></td><td class="right">' + esc(p.confidence) + '</td><td class="right">' + esc(p.autoConfidence) + '</td><td class="right">' + esc(p.effectiveConfidence) + '</td><td class="right">' + esc(p.recentSignalsCount) + '<div class="small">B ' + esc(p.recentBuySignalsCount) + ' / S ' + esc(p.recentSellSignalsCount) + '</div></td><td class="right">' + percent(p.lastReturnPercent) + '</td><td class="nowrap">' + time(p.lastCheckedAt) + '</td><td class="reason">' + esc(p.scoreReason || p.lastError) + '</td></tr>'
      );
